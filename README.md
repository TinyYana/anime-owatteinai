...
可自架、申請制的 Discord 社群追番工具。Discord 登入、成員審核、追番進度、觀看入口，就這樣。沒有播放器、沒有 AI 推薦。

目前第一個實際使用的部署是我自己的彼岸花 Discord 社群；其他社群也可以 fork / 部署一份，換成自己的 Discord Application、伺服器 ID、網域與資料庫來使用。
...
Discord 頻道的追番記錄散落在訊息裡，找不回來，也沒辦法知道群組整體在看什麼。做一個給自己社群用的內部工具比找現有服務再適配容易得多。

申請制不是為了麻煩人，是因為這類工具通常服務一個明確的 Discord 社群；每個自架者可以決定自己的審核標準與使用範圍。
...
這個 repo 不是只能給彼岸花用。彼岸花只是目前第一個正式使用的社群；如果你有自己的 Discord 社群，可以部署一份獨立的追番進行式。

自架時通常需要替換：

- Discord Application：`DISCORD_CLIENT_ID`、`DISCORD_CLIENT_SECRET`、OAuth redirect URI
- Discord 伺服器：`DISCORD_GUILD_ID`、Bot 加入的伺服器、需要自動設為 owner 的 `ADMIN_DISCORD_IDS`
- 部署位置：`APP_BASE_URL`、Cloudflare Workers 自訂網域、瀏覽器插件設定中的 API 來源
- 資料庫與快取：自己的 Neon PostgreSQL、Cloudflare KV namespace、Cloudflare secrets
- 社群文字與品牌：README、Landing page、Terms、OG image、頁尾 Discord 邀請等可依你的社群名稱調整

每個部署的資料彼此獨立；申請審核、角色權限、觀看入口與公開社群統計都以該部署設定的 Discord 伺服器為邊界。
...
- `community`：使用者主動選擇後，該部署的社群成員可見。短評可選 community，但預設 private。
